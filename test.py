...
def add_patient_to_db(document):
    sql = 'SELECT id FROM patients WHERE pib = %s AND birthday = %s AND sex = %s LIMIT 1'
    ...
    if result is not None:
        if len(result) >= 2 and result[1] != document['Номер телефону пацієнта']:
            sql = 'UPDATE patients SET phone = %s WHERE id = %s'
            cursor.execute(sql, (document['Номер телефону пацієнта'], result[0]))
        return result[0]

    data = (document["Ім'я"], document['Прізвище'], document['По батькові'], document['Дата народження пацієнта'],
            document['Номер телефону пацієнта'], document['Стать пацієнта'],
            document['Пацієнт'], document['Дата'])
    query = ("INSERT INTO patients (fname, lname, mname, birthday, phone, sex, pib, created_date) "
